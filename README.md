...
A Websockets Chatbox in Python with no external dependencies.
  ...
  * detailed logs for both the client and the server
...
| `RSA_encryption`            | sends the encrypted message in which public key is attached.
| `RSA_decryption`            | sends the decrypted message which seperates public key and decrpt the message and returns to the client.
...
```
Clients will get real time updates of number of clients that are joining to the server with their id.
